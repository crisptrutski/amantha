(ns amantha.components.generic
  ...
  (:require
    [amantha.utils :as utils :refer [glyphicon]]
    [amantha.utils :refer [glyphicon]]
    [reagent.core :as reagent]
    [re-frame.core :refer [dispatch]]))

(defn toggle-panel [state title hide-key content]
  (let [show (not (get-in @state [:preferences hide-key]))]
    [:div.panel.panel-default
     [:div.panel-heading.clickable
      {:on-click #(reset! state update-in [:preferences hide-key] not)}
      [:div.panel-title
